...
export class SemesterUpdateComponent implements OnInit {
  ...
  constructor( private formBuilder: FormBuilder,
    ...
    async ngOnInit() {
    this.semester = this._semesterService.getsemester();
    this.semesterUpdateForm = this.formBuilder.group({
    ...
    this.basicData.patchValue(this.semester);
  ...
  get basicData() {
    return this.semesterUpdateForm.get('basic_data');
  }

  updateSemester = () => {
    const basicData: any = (this.basicData as FormGroup).getRawValue();
    basicData.cuts = basicData.cuts.map((obj:any)=> { return obj.id});
    this._semesterService.update(this.semester.id,basicData).then((response: any) => {
      this.progress = 1;
