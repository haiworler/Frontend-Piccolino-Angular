...
@Component({
  selector: 'app-enrolled-create',
  templateUrl: './enrolled-create.component.html',
  styles: []
})
export class EnrolledCreateComponent implements OnInit {
  enrolledCreateForm: FormGroup;
  userData: any;
  ...
  async ngOnInit() {
    this.enrolledCreateForm = this.formBuilder.group({
  ...
  get basicData() {
    return this.enrolledCreateForm.get('basic_data');
  }
  ...
  createenrolled = () => {
    ...
    this._enrolledService.create(data).then((response: any) => {
      this.progress = 1;
      this._storageService.setItem('token', localStorage.getItem('token'));
      this._modalService.close();
      this._notificationService.success({
        title: 'Información',
        message: 'La matrícula se ha registrado correctamente.'
      });
    }).catch((response: any) => {
      this.progress = false;
    });
