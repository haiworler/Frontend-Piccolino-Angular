 ...
 * PDF
 */
import { PeopleCertificateService } from '@services/app-services/export/pdf/people-certificate.service';

@Component({
  selector: 'app-voluntary-update',
  templateUrl: './voluntary-update.component.html',
  styles: []
})
export class VoluntaryUpdateComponent implements OnInit {


  voluntaryUpdateForm: FormGroup;
  userData: any;
  ...
  constructor(
    ...
    private _peopleCertificateService: PeopleCertificateService,

  ) { }

  async ngOnInit() {
    this.student = this._peopleService.getpeople();
    this.voluntaryUpdateForm = this.formBuilder.group({
...
// Se crean los conctactos registrados
  ...
  // Se crea el historial de procesos registrados
  this.student.historys.forEach((history:any) => {
    if(history.history_type_id == 1){
      (this.historiesData as FormArray).push(
        this.formBuilder.group({
          id: history.id,
          history_type_id: [history.history_type_id, [Validators.required]],
          process: [history.process, [Validators.required]],
          hours: [history.hours, [Validators.required]],
          start_date: [history.start_date, [Validators.required]],
          end_date: [history.end_date, [Validators.required]],
  ...
  get basicData() {
    return this.voluntaryUpdateForm.get('basic_data');
  }

  get additionalData() {
    return this.voluntaryUpdateForm.get('additional_data');
  }

   
  get contactsData() {
    return this.voluntaryUpdateForm.get('contacts_data');
  }
  get documentsData() {
    return this.voluntaryUpdateForm.get('documents_data');
  }
  get academicInformationsData() {
    return this.voluntaryUpdateForm.get('academic_informations_data');
  }
  get historiesData() {
    return this.voluntaryUpdateForm.get('histories_data');
  }
  ...
  removeHistory = (event: any, Index: number) => {
    event.preventDefault();
    let objArray: any = (this.historiesData as FormArray).at(Index).value;
    if (objArray.id) {
      (this.historiesData as FormArray).at(Index).get('state').setValue(false);
 ...
 /**
  ...
  generatePDF(){
    this._peopleCertificateService.generatePDF(this.student);
  }
